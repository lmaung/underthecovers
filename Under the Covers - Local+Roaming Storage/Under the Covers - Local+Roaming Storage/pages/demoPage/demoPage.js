...
(function () {
    ...
    WinJS.UI.Pages.define("/pages/demoPage/demoPage.html", {
        ...
        ready: function (element, options) {
            ...
            var inputRoamText = document.getElementById("inputRoamText");
            inputRoamText.addEventListener("change", function (e) {
                roamingSettings.values["savedRoamingValue"] = inputRoamText.value;
                a = 1;
            });

            var inputLocalText = document.getElementById("inputLocalText");
            inputLocalText.addEventListener("change", function (e) {
                localSettings.values["savedLocalValue"] = inputLocalText.value;
            });
    ...
    function reloadSavedData() {
        var localValue = localSettings.values["savedLocalValue"];
        if (localValue) {
            document.getElementById("inputLocalText").innerText = localValue;
        }

        var roamingValue = roamingSettings.values["savedRoamingValue"];
        if (roamingValue) {
            document.getElementById("inputRoamText").innerText = roamingValue;
        }
    }
